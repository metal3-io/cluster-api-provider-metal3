...
func (r *Metal3MachineReconciler) Reconcile(ctx context.Context, req ctrl.Request) (_ ctrl.Result, rerr error) {
	...
	if err := r.Client.Get(ctx, req.NamespacedName, capm3Machine); err != nil {
		if apierrors.IsNotFound(err) {
			logLogicGate(machineLog, "Metal3Machine resource not found, skipping")
			return ctrl.Result{}, nil
		}
		return ctrl.Result{}, errors.Wrap(err, "Failed to fetch Metal3Machine")
	}
	...
	if err != nil {
		setErrorM3Machine(capm3Machine, "Machine is missing cluster label or cluster does not exist", capierrors.InvalidConfigurationMachineError)
		return ctrl.Result{}, nil
	}
	...
	if setPause {
		...
		if err != nil {
			machineLog.Error(err, "Failed to set pause annotation on associated BareMetalHost")
		...
		if err != nil {
			machineLog.Error(err, "Failed to remove pause annotation on associated BareMetalHost")
	...
	if !capm3Machine.ObjectMeta.DeletionTimestamp.IsZero() {
		logLogicGate(machineLog, "Metal3Machine has deletion timestamp, entering delete reconciliation")
		return r.reconcileDelete(ctx, machineMgr, machineLog)
	}

	// Handle non-deleted machines
	logLogicGate(machineLog, "Metal3Machine is active, entering normal reconciliation")
...
func (r *Metal3MachineReconciler) reconcileNormal(ctx context.Context,
	...
	logLogicGate(machineLog, "Ensuring Metal3Machine finalizer is present")
	...
	if machineMgr.IsProvisioned() && machineMgr.MachineHasNodeRef() {
		logLogicGate(machineLog, "Machine already provisioned with NodeRef, updating status")
	...
	if !machineMgr.IsBootstrapReady() {
		logLogicGate(machineLog, "Bootstrap data not ready, waiting")
	...
	if !machineMgr.HasAnnotation() {
		logLogicGate(machineLog, "Metal3Machine has no host annotation, attempting association")
		// Associate the baremetalhost hosting the machine
		err := machineMgr.Associate(ctx)
		if err != nil {
			machineLog.Error(err, "Failed to associate BareMetalHost")
	...
	logLogicGate(machineLog, "Metal3Machine associated with BareMetalHost")

	// Make sure that the metadata is ready if any
	err := machineMgr.AssociateM3Metadata(ctx)
	if err != nil {
		machineLog.Error(err, "Failed to associate Metal3 metadata")
	...
	logLogicGate(machineLog, "Metal3 metadata is associated")
	...
	if machineMgr.NodeWithMatchingProviderIDExists(ctx, r.CapiClientGetter) {
		...
		logLogicGate(machineLog, "Node with matching ProviderID already exists, waiting for NodeRef")
		machineMgr.SetReadyTrue()
		return ctrl.Result{}, nil
	}

	if machineMgr.CloudProviderEnabled() {
		logLogicGate(machineLog, "Cloud provider integration enabled, attempting ProviderID sync from cloud node")
	...
	// caught it above.
	logLogicGate(machineLog, "Attempting to derive ProviderID from Node label")
	...
	if success {
		machineMgr.SetReadyTrue()
		logLogicGate(machineLog, "Successfully set ProviderID from Node label")
		return ctrl.Result{}, nil
	}

	if !machineMgr.Metal3MachineHasProviderID() {
		logLogicGate(machineLog, "Setting default ProviderID as none was present")
	...
	logLogicGate(machineLog, "Set node ProviderID by hostname")
	...
	logLogicGate(machineLog, "Metal3Machine reconciliation finished successfully")
...
func (r *Metal3MachineReconciler) reconcileDelete(ctx context.Context,
	...
	logLogicGate(machineLog, "Setting Metal3Machine deleting conditions")
	...
	logLogicGate(machineLog, "Dissociating metadata")
	if err := machineMgr.DissociateM3Metadata(ctx); err != nil {
		machineLog.Error(err, "Failed to dissociate metadata")
	...
	// delete the machine
	logLogicGate(machineLog, "Deleting Metal3Machine resources")
	if err := machineMgr.Delete(ctx); err != nil {
		machineLog.Error(err, "Failed to delete Metal3Machine")
	...
	logLogicGate(machineLog, "Removing Metal3Machine finalizer after delete")
	machineMgr.UnsetFinalizer()
	logLogicGate(machineLog, "Delete reconciliation completed")
