...
generate_vbmctl_vms() {
  ...
  # VM sizing is configurable so lightweight scenarios don't allocate the full
  # default footprint per node.
  local vm_memory="${VM_MEMORY:-4096}"
  local vm_vcpus="${VM_VCPUS:-2}"
  local vm_disk_size="${VM_DISK_SIZE:-20}"
...
ensure_libvirt_access() {
  ...
  if sudo virsh -c "${uri}" uri &>/dev/null; then
    return 0
  fi
  ...
  if command -v systemctl &>/dev/null; then
    ...
    if systemctl cat virtqemud.socket &>/dev/null; then
...
# Run under sudo: vbmctl creates veth pairs (needs CAP_NET_ADMIN). On Leap the
# setcap that grants the binary that capability doesn't stick (the file
# capability write doesn't take effect), so vbmctl otherwise fails with
# "operation not permitted". sudo grants the caps directly, independent of
# setcap.
...
# so Ironic in the kind cluster can't reach the BMCs. Move them to 'trusted'.
# No-op where firewalld isn't running (e.g. Ubuntu).
if command -v firewall-cmd &>/dev/null && sudo firewall-cmd --state &>/dev/null; then
  echo "firewalld is active; moving lab bridges to the 'trusted' zone..."
  for bridge in "${PROVISIONING_BRIDGE}" "${EXTERNAL_BRIDGE}"; do
    sudo firewall-cmd --zone=trusted --change-interface="${bridge}" || true
  done
fi
