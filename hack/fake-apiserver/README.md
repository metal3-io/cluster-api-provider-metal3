...
You can build the fake-api-server image that is suitable for your local environment with
...
For local tests, it's normally needed to load the image into the cluster. For e.g. with `minikube`
...
After the cluster is created, information like node name and provider ID can be added to the
fake API server by sending a GET request to `/updateNode` endpoint:
