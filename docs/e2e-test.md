...
| `KUBERNETES_VERSION_PATCH_FROM` | Starting version for patch upgrades | v1.33.4 | node-reuse (patch upgrade tests) |
...
**Note:** For patch upgrade tests (node-reuse), the target version is `KUBERNETES_VERSION`.
The `KUBERNETES_VERSION_PATCH_FROM` variable specifies only the starting patch version.
