...
.PHONY: deploy-test-extension
deploy-test-extension: ## Deploy test extension to the cluster
	kubectl apply -k test/extension/config/

.PHONY: delete-test-extension
delete-test-extension: ## Delete test extension from the cluster
	kubectl delete -k test/extension/config/ || true
...
.PHONY: release-test-extension-manifests
release-test-extension-manifests: $(KUSTOMIZE) $(RELEASE_DIR) ## Builds the runtime extension manifests for development
	$(KUSTOMIZE) build test/extension/config/default > $(RELEASE_DIR)/runtime-extension-components-development.yaml
