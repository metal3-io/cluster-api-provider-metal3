...
clean-e2e: ## Clean up e2e artifacts, kind clusters, and leftover resources
	...
	# Force-remove any leftover kind node containers. These can linger after an
	# interrupted run even when "kind get clusters" no longer lists the cluster,
	# causing "node(s) already exist for a cluster" on the next run.
	@for node in $$(docker ps -aq --filter "label=io.x-k8s.kind.cluster" 2>/dev/null); do \
	...
	# Restore envsubst-processed template files
	cd $(ROOT_DIR)/test/e2e && git checkout -- data/bmo-deployment/ data/ironic-standalone-operator/ 2>/dev/null || true
