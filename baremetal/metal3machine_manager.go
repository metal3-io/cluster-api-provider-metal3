...
func (m *MachineManager) UsingKubeadm() bool {
	// TODO: is this enough?
	return m.Machine.Spec.Bootstrap.ConfigRef != nil
}
...
func (m *MachineManager) getMetal3MachineHostname() string {
...
func (m *MachineManager) SetDefaultProviderID() {
...
func (m *MachineManager) SetProviderIDFromNodeLabel(ctx context.Context, clientFactory ClientGetter) error {
	...
	return fmt.Errorf("no nodes with matching label")
}
...
func (m *MachineManager) SetNodeProviderIDByHostname(ctx context.Context, clientFactory ClientGetter) error {
	...
	metal3MachineHostname := m.getMetal3MachineHostname()

	if metal3MachineHostname == "" {
	...
	filter := metav1.ListOptions{
		LabelSelector: fmt.Sprintf("kubernetes.io/hostname=%s", metal3MachineHostname),
	}
	nodes, err := corev1Remote.Nodes().List(ctx, filter)
	...
	if len(nodes.Items) == 0 {
		errMessage := fmt.Sprintf("unable to find a Node with hostname: %s", metal3MachineHostname)
		m.Log.Info(errMessage)
		return WithTransientError(errors.New(errMessage), requeueAfter)
	}

	if len(nodes.Items) > 1 {
		errMessage := fmt.Sprintf("more than one Node with hostname: %s", metal3MachineHostname)
		m.Log.Info(errMessage)
		return WithTransientError(errors.New(errMessage), requeueAfter)
	}
...
func (m *MachineManager) getNodeByProviderID(ctx context.Context, providerIDLegacy, providerIDNew string, clientFactory ClientGetter) (corev1.Node, error) {
	...
	return corev1.Node{}, fmt.Errorf("unable to find node with matching ProviderID")
}
