...
// It uses the SSH key from /root/.ssh/id_rsa (mounted via kustomize secret).
func runCommand(machineIP, user, command string) (string, error) {
	...
	if _, err := os.Stat(keyPath); os.IsNotExist(err) {
		return "", fmt.Errorf("SSH private key not found at %s. Ensure the ssh-key secret is mounted correctly via kustomize. Deploy with: export SSH_KEY_PATH=~/.ssh/id_rsa && kubectl apply -k test/extension/config/", keyPath)
	}
	...
	if err := session.Run(fullCommand + "\n"); err != nil {
...
EOF'`, majorMinor, majorMinor)
	...
	// For worker nodes, this step might be different or skipped
