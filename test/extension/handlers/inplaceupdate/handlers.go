...
func canUpdateMetal3MachineSpec(current, desired *infrav1.Metal3MachineSpec) {
...
func (h *ExtensionHandlers) DoUpdateMachine(ctx context.Context, req *runtimehooksv1.UpdateMachineRequest, resp *runtimehooksv1.UpdateMachineResponse) {
	...
	machineIP, err := h.getMachineIP(ctx, &req.Desired.Machine, "externalv4-pool")
	if err != nil {
	...
	// Check if this is the first time being called for this machine
	if _, ok := h.state.Load(key); ok {
		// Not the first call - check if the upgrade is complete via SSH
		user := "metal3"
	...
	// First time called - start the upgrade process
	h.state.Store(key, true)

	log.Info("Starting in-place Kubernetes upgrade", "machineIP", machineIP, "targetVersion", targetVersion)

	// Perform the actual upgrade via SSH
	// Using "metal3" as the default user - adjust if needed
	user := "metal3"
	if err := upgradeKubernetesInPlace(machineIP, user, targetVersion); err != nil {
		log.Error(err, "Failed to perform in-place upgrade")
		h.state.Delete(key) // Clean up state on failure
		resp.Status = runtimehooksv1.ResponseStatusFailure
		resp.Message = fmt.Sprintf("Failed to upgrade Kubernetes: %v", err)
		return
	}

	log.Info("In-place Kubernetes upgrade initiated successfully")
