...
func Pivoting(ctx context.Context, inputGetter func() PivotingInput) {
	...
	// The bootstrap kind node statically holds the Ironic provisioning VIP on the
	// provisioning interface (added at suite setup). Now that Ironic has moved to the
	// self-hosted target cluster, that stale address makes the kind node answer ARP for
	// the VIP while nothing listens on it, so BMHs get intermittent connection-refused.
	By("Removing provisioning network from bootstrap cluster after moving Ironic to target")
	provisioningInterface := input.E2EConfig.MustGetVariable("BARE_METAL_PROVISIONER_INTERFACE")
	RemoveProvisioningNetwork(ctx, input.E2EConfig.ManagementClusterName, provisioningInterface)
...
func RePivoting(ctx context.Context, inputGetter func() RePivotingInput) {
	...
	err := FetchClusterLogs(input.TargetCluster, filepath.Join(input.ArtifactFolder, "afterPivot"))
	if err != nil {
		Logf("Error: %v", err)
	}

	By("Fetch manifest for workload cluster after pivot")
	err = FetchManifests(input.TargetCluster, filepath.Join(input.ArtifactFolder, "afterPivot"))
	if err != nil {
		Logf("Error fetching manifests for workload cluster after pivot: %v", err)
	}

	By("Fetch logs from bootstrap cluster after pivot")
	err = FetchClusterLogs(input.BootstrapClusterProxy, filepath.Join(input.ArtifactFolder, "afterPivot"))
	if err != nil {
		Logf("Error: %v", err)
	}

	By("Fetch manifest for bootstrap cluster after pivot")
	err = FetchManifests(input.BootstrapClusterProxy, filepath.Join(input.ArtifactFolder, "afterPivot"))
	if err != nil {
		Logf("Error fetching manifests for bootstrap cluster after pivot: %v", err)
	}
	...
	// Restore the provisioning VIP on the bootstrap kind node before Ironic is reinstalled
	// there, so its keepalived has the interface address again (mirrors the removal done
	// when pivoting to the target cluster).
