...
func generateTemplateFromHelmChart(ctx context.Context, opts HelmOpts, manifestOverwriteValues map[string]interface{}) (string, error) {
	actionConfig, err := initActionConfig(opts)
	if err != nil {
		return "", fmt.Errorf("failed to init action config: %w", err)
	}

	kubeversion, err := chartutil.ParseKubeVersion(os.Getenv("KUBERNETES_VERSION"))
	if err != nil {
