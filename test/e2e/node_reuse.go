...
func verifyMetal3MachineNodeReuseCondition(ctx context.Context, managementClusterClient client.Client, bmhKey types.NamespacedName, namespace string, intervals []interface{}) {
	...
	Eventually(
		func(g Gomega) {
			...
			// Check AssociateBareMetalHostCondition reason
			cond := conditions.Get(&m3m, infrav1.AssociateBareMetalHostCondition)
			g.Expect(cond).NotTo(BeNil(), "AssociateBareMetalHostCondition not found")
			g.Expect(cond.Status).To(Equal(metav1.ConditionTrue), "AssociateBareMetalHostCondition should be True")
			g.Expect(cond.Reason).To(Equal(infrav1.AssociateBareMetalHostViaNodeReuseSuccessReason),
				"Expected AssociateBareMetalHostViaNodeReuseSuccessReason but got %s", cond.Reason)
		}, intervals...,
	).Should(Succeed())
}
