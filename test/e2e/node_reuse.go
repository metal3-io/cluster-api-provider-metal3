...
func NodeReuse(ctx context.Context, inputGetter func() NodeReuseInput) {
	...
	pointMDtoM3mt(ctx, input.Namespace, input.ClusterName, m3MachineTemplateName, machineDeploy.Name, clusterClient)
...
func updateNodeReuse(ctx context.Context, namespace string, nodeReuse bool, m3MachineTemplateName string, clusterClient client.Client) {
	...
	helper, err := v1beta1patch.NewHelper(&m3machineTemplate, clusterClient)
	Expect(err).NotTo(HaveOccurred())
...
func pointMDtoM3mt(ctx context.Context, namespace string, clusterName string, m3mtname, mdName string, clusterClient client.Client) {
	md := clusterv1.MachineDeployment{}
	Expect(clusterClient.Get(ctx, client.ObjectKey{Namespace: namespace, Name: mdName}, &md)).To(Succeed())
	helper, err := v1beta1patch.NewHelper(&md, clusterClient)
	Expect(err).NotTo(HaveOccurred())
