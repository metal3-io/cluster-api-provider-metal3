...
func NodeReuse(ctx context.Context, inputGetter func() NodeReuseInput) {
	...
	err = FetchClusterLogs(input.ClusterProxy, filepath.Join(input.ArtifactFolder, input.ClusterProxy.GetName(), "node_reuse", "preUpgrade-kcp", "logs"))
	if err != nil {
		Logf("Error: %v", err)
	}

	err = FetchManifests(input.ClusterProxy, filepath.Join(input.ArtifactFolder, input.ClusterProxy.GetName(), "node_reuse", "preUpgrade-kcp", "manifests"))
	if err != nil {
	...
	err = FetchClusterLogs(input.ClusterProxy, filepath.Join(input.ArtifactFolder, input.ClusterProxy.GetName(), "node_reuse", "postUpgrade-kcp", "logs"))
	if err != nil {
		Logf("Error: %v", err)
	}
	err = FetchManifests(input.ClusterProxy, filepath.Join(input.ArtifactFolder, input.ClusterProxy.GetName(), "node_reuse", "postUpgrade-kcp", "manifests"))
	if err != nil {
		Logf("Error fetching manifests: %v", err)
	}
	...
	err = FetchClusterLogs(input.ClusterProxy, filepath.Join(input.ArtifactFolder, input.ClusterProxy.GetName(), "node_reuse", "preUpgrade-worker", "logs"))
	if err != nil {
		Logf("Error: %v", err)
	}
	err = FetchManifests(input.ClusterProxy, filepath.Join(input.ArtifactFolder, input.ClusterProxy.GetName(), "node_reuse", "preUpgrade-worker", "manifests"))
	if err != nil {
		Logf("Error fetching manifests: %v", err)
	}
	...
	err = FetchClusterLogs(input.ClusterProxy, filepath.Join(input.ArtifactFolder, input.ClusterProxy.GetName(), "node_reuse", "postUpgrade-worker", "logs"))
	if err != nil {
		Logf("Error: %v", err)
	}
	err = FetchManifests(input.ClusterProxy, filepath.Join(input.ArtifactFolder, input.ClusterProxy.GetName(), "node_reuse", "postUpgrade-worker", "manifests"))
	if err != nil {
		Logf("Error fetching manifests: %v", err)
	}
